<script lang="ts">
	...
	import {
		...
		getActiveCursor,
		getCompletedCursor,
		getActiveCursorTriplet,
		getCompletedCursorTriplet,
		getActiveTradesChain,
		getCompletedTradesChain,
		fetchActiveTrades,
	...
	let {
		...
		selectedAddress = '',
		onnavigate = () => {}
	}: { selectedChain?: string; selectedAddress?: string; onnavigate?: () => void } = $props();
	...
	let editTargets = $state<
		{
			id?: string;
			type: string;
			trigger: string;
			sellPct: string;
			targetKind: 'TAKE_PROFIT' | 'STOP_LOSS';
	...
	let editStopLoss = $state<{
		...
		trigger: string;
		sellPct: string;
	}>({ enabled: false, mode: 'NORMAL', type: 'PERCENTAGE', trigger: '50', sellPct: '100' });
	...
	function pendingBuyAmount(
		trade: ActiveTrade | CompletedTrade
	): { type: 'USD' | 'NATIVE'; value: number } | null {
		if (activeTab !== 'pending') return null;
		const buys = (
			((trade as ActiveTrade).pendingSwaps ?? []) as {
				side: 'BUY' | 'SELL';
				amount?: { type: 'USD' | 'NATIVE'; value: number };
			}[]
		).filter((s) => s.side === 'BUY' && s.amount);
	...
	function navigateToTrade(trade: ActiveTrade | CompletedTrade) {
		...
		expandedTradeIds = next;
		onnavigate();
		goto(`/?chain=${trade.chain}&token=${trade.tokenAddress}`, {
			replaceState: false,
			noScroll: true
		});
	}
	...
	function triggerValue(trigger: TradeTargetTrigger): string {
		switch (trigger.type) {
			case 'MULTIPLIER':
				return String(trigger.multiplier);
			case 'PERCENT':
				return String(trigger.changePct);
			case 'MARKET_CAP_USD':
				return String(trigger.marketCapUsd);
			case 'PRICE':
				return String(trigger.priceUsd);
		}
	}
	...
	function targetTriggerLabel(target: TradeTarget | TradeMetTarget): string {
		switch (target.trigger.type) {
			case 'MULTIPLIER':
				return `${Number(target.trigger.multiplier).toFixed(target.kind === 'STOP_LOSS' ? 2 : 1)}x`;
			case 'PERCENT': {
				const sign = target.kind === 'STOP_LOSS' ? '-' : '+';
				return `${sign}${Math.abs(Number(target.trigger.changePct)).toFixed(0)}%`;
			}
			case 'MARKET_CAP_USD':
				return formatMarketCap(target.trigger.marketCapUsd);
			case 'PRICE':
				return formatUsd(String(target.trigger.priceUsd));
		}
	}
	...
	function addEditTarget(
		kind: 'TAKE_PROFIT' | 'STOP_LOSS' = 'TAKE_PROFIT',
		mode: 'NORMAL' | 'TRAILING' = 'NORMAL'
	) {
		const trailing = mode === 'TRAILING';
		editTargets = [
			...editTargets,
			{
				type: trailing ? 'PERCENTAGE' : 'MULTIPLE',
				trigger: kind === 'TAKE_PROFIT' ? '2' : trailing ? '20' : '50',
				sellPct: kind === 'TAKE_PROFIT' ? '50' : '100',
				targetKind: trailing ? 'STOP_LOSS' : kind,
	...
	function buildTargetPayload(t: {
		type: string;
		trigger: string;
		sellPct: string;
		targetKind: 'TAKE_PROFIT' | 'STOP_LOSS';
		mode: 'NORMAL' | 'TRAILING';
	}): TradeTargetConfig | null {
		const sellPct = parseFloat(t.sellPct || '0');
		let trigger: TradeTargetTrigger;
		switch (t.type) {
			case 'MULTIPLE':
				trigger = { type: 'MULTIPLIER', multiplier: parseFloat(t.trigger) };
				break;
			case 'PERCENTAGE':
				trigger = positivePercentTargetTrigger(parseFloat(t.trigger));
				break;
			case 'MARKETCAP':
				trigger = { type: 'MARKET_CAP_USD', marketCapUsd: parseFloat(t.trigger) };
				break;
			case 'USD':
				trigger = { type: 'PRICE', priceUsd: parseFloat(t.trigger) };
				break;
			default:
				return null;
		}
	...
	async function savePositionEdit() {
		...
		try {
			...
			for (const id of existingIds) {
				if (!keptIds.has(id)) {
					await api.DELETE('/v2/trade/{chain}/{token}/targets/remove/{id}', {
						params: { path: { chain, token, id } }
					});
				}
			}
			for (const t of editTargets) {
				...
				if (t.id && existingIds.has(t.id)) {
					await api.POST('/v2/trade/{chain}/{token}/targets/update/{id}', {
						params: { path: { chain, token, id: t.id } },
						body: payload
					});
				} else {
					await api.POST('/v2/trade/{chain}/{token}/targets/add', {
						params: { path: { chain, token } },
						body: payload
					});
				}
			}
			const sellSlippagePct =
				editSellSlippage && parseFloat(editSellSlippage) > 0
					? parseFloat(editSellSlippage)
					: ('AUTO' as const);
			...
			await api.POST('/v2/trade/{chain}/{token}/settings', {
				params: { path: { chain, token } },
	...
	function setupActiveTradesWs() {
		...
		const windowParams = liveAccumulatedParams(getActiveCursorTriplet());
		if (!windowParams) return;
		const chain = getActiveTradesChain();
		const params = { ...windowParams, ...(chain ? { chain } : {}) };
		activeTradesWsKey = subscribe(
			'trades:active',
			(event, data, topic, meta) => {
			...
			params,
			{
				recovery: 'refetch',
				onReconnect: () => {
					void fetchActiveTrades(undefined, chain).then(setupActiveTradesWs);
				}
			}
		);
	}
	...
	function setupCompletedTradesWs() {
		...
		const windowParams = liveAccumulatedParams(getCompletedCursorTriplet());
		if (!windowParams) return;
		const chain = getCompletedTradesChain();
		const params = { ...windowParams, ...(chain ? { chain } : {}) };
		completedTradesWsKey = subscribe(
			'trades:completed',
			(event, data, topic, meta) => {
				if (event === 'USER_TRADES') handleTradesSnapshot(topic, data, meta);
				if (event === 'USER_TRADES_UPDATE') handleTradesUpdate(data, meta);
			...
			params,
			{
				recovery: 'refetch',
				onReconnect: () => {
					void fetchCompletedTrades(undefined, chain).then(setupCompletedTradesWs);
				}
			}
		);
	}
	...
	function setupTradeWs() {
		...
		tradeWsKey = subscribe('user', (event, data, _topic, meta) => {
			if (event === 'USER_TRADE_UPDATE' || event === 'USER_TRADE_UPDATE_ERROR')
				handleTradeUpdate(event, data, meta);
	...
	$effect(() => {
		let cancelled = false;
		if (getIsLoggedIn()) {
			fetchActiveTrades().then(() => {
				if (!cancelled) setupActiveTradesWs();
			});
			setupTradeWs();
		}
		return () => {
	...
	$effect(() => {
		...
		if (activeTab === 'history' && getIsLoggedIn() && !getCompletedFetched()) {
			fetchCompletedTrades().then(() => {
				if (!cancelled) setupCompletedTradesWs();
			});
		}
		return () => {
			cancelled = true;
	...
	function statusBadge(status: string): string {
		switch (status) {
			case 'ACTIVE':
				return 'bg-grn/20 text-grn';
			case 'PENDING':
				return 'bg-yel/20 text-yel';
			case 'COMPLETED':
				return 'bg-g4/20 text-g5';
			default:
				return 'bg-g4/20 text-g5';
		}
	}
	...
	function displayedTradeTimestamp(trade: ActiveTrade | CompletedTrade): number {
		return trade.status === 'COMPLETED' ? completedTradeTimestamp(trade) : trade.createdAtTimestamp;
	}
...
<div class="flex h-full flex-col border-t border-bd bg-transparent">
	<div class="flex border-b border-bd">
		{#each tabs as tab}
			...
			>
				{tab.label}
				{#if count > 0}
					<span class="ml-1 rounded-full bg-s7 px-1.5 py-0.5 text-[10px] font-medium text-g5"
						>{count}</span
					>
				{/if}
				{#if activeTab === tab.value}
					<div
						class="absolute bottom-0 left-1/2 h-[2px] w-6 -translate-x-1/2 rounded-full bg-grn"
					></div>
				{/if}
			</button>
		{/each}
		...
		<div class="flex-1 overflow-y-auto p-2.5">
			<div class="space-y-1.5">
				{#each currentTrades() as trade (trade.id)}
					...
					<div
						class="group cursor-pointer rounded-xl border transition-all {isViewing(trade)
							? 'border-grn/40 bg-s1'
							: 'border-bd bg-s1 hover:border-bd3'}"
					...
					>
						<div class="flex cursor-pointer items-center gap-2 p-2.5">
							...
							<div class="min-w-0 flex-1">
								<div class="flex items-center gap-1.5">
									...
									<span
										class="shrink-0 cursor-pointer rounded bg-s7 px-1 py-0.5 text-[10px] transition-colors {copiedTradeId ===
										trade.id
											? 'text-grn'
											: 'text-g4 hover:text-g9'}"
										onclick={(e) => {
											e.stopPropagation();
											navigator.clipboard.writeText(String(trade.id));
											copiedTradeId = trade.id;
											setTimeout(() => {
												if (copiedTradeId === trade.id) copiedTradeId = null;
											}, 1500);
										}}
										title="Copy trade ID"
							...
							<div class="shrink-0 text-right">
								{#if activeTab === 'pending'}
									...
									<span
										class="inline-flex items-center gap-1 rounded-md bg-yel/10 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-yel"
									>
									...
									{#if pba}
										<div
											class="mt-0.5 flex items-center justify-end gap-0.5 text-sm font-bold text-yel"
										>
											{#if pba.type === 'USD'}
												{formatPriceText(pba.value)}
											{:else}
												{formatNumber(String(pba.value))}
												<ChainIcon chain={trade.chain} class="h-3.5 w-3.5" />
											{/if}
										</div>
									{/if}
								{:else}
									<CurrencyValue
										usd={String(netPnlUsd(trade))}
										native={String(netPnlNative(trade))}
										chain={trade.chain}
										mode="value"
										class="{pnlColor(trade)} text-sm font-bold"
							...
							{#if activeTab === 'history'}
								<button
									...
									onclick={(e) => {
										...
										showShareCard = true;
										if (!PnlShareCard) {
											import('./PnlShareCard.svelte').then((m) => {
												PnlShareCard = m.default;
											});
										}
									}}
									title="Share PnL"
						...
						{#if isExpanded}
							<div class="px-2.5 pb-2.5">
									...
									<div class="mt-1 space-y-px">
										{#each trade.swaps as swap}
											<div class="flex items-center gap-1.5 px-1 py-0.5 text-[10px]">
												<span
													class="w-7 shrink-0 font-bold {swap.side === 'BUY'
														? 'text-grn'
														: 'text-red'}">{swap.side === 'BUY' ? 'BUY' : 'SELL'}</span
												>
												<CurrencyValue
													usd={String(swap.value.amount.usd)}
													native={String(swap.value.amount.native)}
													chain={trade.chain}
													mode="value"
													class="text-tx"
													iconClass="h-3 w-3"
												...
												{#if swap.target}
													<span
														class="rounded bg-{swap.target.kind === 'TAKE_PROFIT'
															? 'grn'
															: 'red'}/10 px-1 py-px text-[9px] font-medium text-{swap.target
															.kind === 'TAKE_PROFIT'
															? 'grn'
															: 'red'}">{swap.target.kind === 'TAKE_PROFIT' ? 'TP' : 'SL'}</span
													>
												{/if}
												<span
													class="ml-auto text-g4 cursor-help"
													title={fullDateTime(swap.timestamp)}
													>{timeAgo(swap.timestamp, getNow())}</span
												>
												{#if !swap.txHash}
													<span class="h-1.5 w-1.5 rounded-full bg-yel animate-pulse"></span>
												{/if}
								...
								{/if}
								{#if activeTab !== 'pending'}
									<div class="mt-1 rounded-lg border border-bd/40 bg-s2 p-2 space-y-1 text-[10px]">
										<div class="flex items-center justify-between">
											<span class="text-g4">Bought</span>
											<span class="font-semibold text-tx"
												><CurrencyValue
													usd={String(trade.totalBought.usd)}
													native={String(trade.totalBought.native)}
													chain={trade.chain}
													mode="value"
													iconClass="h-3 w-3"
												/>
												<span class="text-g4"
													>({formatNumber(String(trade.totalBought.tokens))})</span
												></span
											>
										</div>
										<div class="flex items-center justify-between">
											...
											<span class="font-semibold text-tx"
												><CurrencyValue
													usd={String(trade.totalSold.usd)}
													native={String(trade.totalSold.native)}
													chain={trade.chain}
													mode="value"
													iconClass="h-3 w-3"
												/>
												<span class="text-g4">({formatNumber(String(trade.totalSold.tokens))})</span
												></span
											>
										</div>
										{#if 'currentValue' in trade && trade.currentValue}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold text-tx"
													><CurrencyValue
														usd={String(trade.currentValue.usd)}
														native={String(trade.currentValue.native)}
														chain={trade.chain}
														mode="value"
														iconClass="h-3 w-3"
													/>
													{#if trade.pnl.multiplier}<span class="text-yel"
															>{netPnlMultiplier(trade).toFixed(2)}x</span
														>{/if}</span
												>
											</div>
										{:else if trade.pnl}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold {pnlColor(trade)}"
													><CurrencyValue
														usd={String(netPnlUsd(trade))}
														native={String(netPnlNative(trade))}
														chain={trade.chain}
								...
								{#if activeTab === 'active' || activeTab === 'pending'}
									...
									<div class="mt-1 rounded-lg border border-bd/40 bg-s2 p-2 space-y-1 text-[10px]">
										{#if trade.avgEntryPrice.usd}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold text-tx"
													>{#if entryMc}{formatMarketCap(entryMc)}{/if}
													<span class="text-g4"
														>(<CurrencyValue
															usd={String(trade.avgEntryPrice.usd)}
															native={String(trade.avgEntryPrice.native)}
															chain={trade.chain}
															mode="price"
															iconClass="h-3 w-3"
														/>)</span
										...
										{#if 'currentPrice' in activeTrade && activeTrade.currentPrice?.usd}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold text-tx"
													>{#if currentMc}{formatMarketCap(currentMc)}{/if}
													<span class="text-g4"
														>(<CurrencyValue
															usd={String(activeTrade.currentPrice.usd)}
															native={String(activeTrade.currentPrice.native)}
															chain={trade.chain}
															mode="price"
															iconClass="h-3 w-3"
														/>)</span
										...
										{#if activeTab === 'pending'}
											...
											{#if pb && pb.side === 'BUY' && 'strategy' in pb && pb.strategy.type === 'LIMIT'}
												<div class="flex items-center justify-between">
													...
													<span class="font-semibold text-blu-light"
														>{formatPriceText(pb.strategy.priceUsd)}</span
													>
												</div>
											{/if}
										{/if}
										{#if trade.tokensRemaining > 0}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold text-tx"
													>{formatNumber(String(trade.tokensRemaining))} {trade.tokenSymbol}</span
												>
											</div>
										{/if}
										{#if trade.totalFees.usd > 0}
											<div class="flex items-center justify-between">
												...
												<CurrencyValue
													usd={trade.totalFees.usdStr}
													native={trade.totalFees.nativeStr}
													chain={trade.chain}
													mode="value"
													class="font-semibold text-g6"
													iconClass="h-3 w-3"
												/>
											</div>
										{/if}
									...
									{#if activeTrade.pendingSwaps?.length > 0}
										<div class="mt-1.5 flex items-center gap-1.5 rounded-md bg-yel/10 px-2 py-1">
											...
											<span class="text-[10px] font-medium text-yel"
												>{activeTrade.pendingSwaps.length} pending {activeTrade.pendingSwaps
													.length === 1
													? 'swap'
													: 'swaps'}</span
											>
									...
									{@const hasContent =
										metTakeProfitTargets.length > 0 ||
										metStopLossTargets.length > 0 ||
										liveTakeProfitTargets.length > 0 ||
										liveStopLossTargets.length > 0 ||
										(activeTab === 'pending' && activeTrade.pendingSwaps?.length > 0)}
									{#if hasContent}
										<div class="mt-2 flex flex-wrap gap-1.5">
											{#if metTakeProfitTargets.length > 0}
												{#each metTakeProfitTargets as met, idx}
													...
													<div class="flex items-center gap-1.5 rounded-lg bg-org/10 px-2.5 py-1.5">
														<span
															class="rounded bg-org/20 px-1 py-px text-[9px] font-bold uppercase text-org"
															>filled</span
														>
														<span class="text-[11px] font-bold text-org">TP{idx + 1}</span>
														<span
															class="text-[13px] font-bold text-org line-through decoration-org/30"
															>{targetTriggerLabel(met)}</span
														>
											...
											{#if metStopLossTargets.length > 0}
												{#each metStopLossTargets as met, idx}
													...
													<div class="flex items-center gap-1.5 rounded-lg bg-red/10 px-2.5 py-1.5">
														<span
															class="rounded bg-red/20 px-1 py-px text-[9px] font-bold uppercase text-red"
															>filled</span
														>
														<span class="text-[11px] font-bold text-red">SL{idx + 1}</span>
														<span
															class="text-[13px] font-bold text-red line-through decoration-red/30"
															>{targetTriggerLabel(met)}</span
														>
											...
											{#each liveTakeProfitTargets as target, idx}
												{@const mult =
													target.trigger.type === 'MULTIPLIER'
														? Number(target.trigger.multiplier)
														: 0}
												{@const pctSell = Math.round(target.sellPct)}
												{@const targetMcVal =
													entryMc && target.trigger.type === 'MULTIPLIER' ? entryMc * mult : null}
												<div class="flex items-center gap-1.5 rounded-lg bg-grn/10 px-2.5 py-1.5">
													<span class="text-[11px] font-bold text-grn"
														>TP{metTakeProfitTargets.length + idx + 1}</span
													>
													<span class="text-[13px] font-bold text-grn">
														{targetTriggerLabel(target)}
													</span>
													{#if targetMcVal}
														<span class="text-[10px] text-grn/40"
															>{formatMarketCap(targetMcVal)}</span
														>
													{/if}
											...
											{#each liveStopLossTargets as sl, idx}
												...
												<div class="flex items-center gap-1.5 rounded-lg bg-red/10 px-2.5 py-1.5">
													<span class="text-[11px] font-bold text-red"
														>SL{metStopLossTargets.length + idx + 1}</span
													>
													<span class="text-[13px] font-bold text-red">
														{targetTriggerLabel(sl)}
													</span>
													{#if sl.mode === 'TRAILING'}<span class="text-[11px] text-red/50"
															>trail</span
														>{/if}
												...
												{#each activeTrade.pendingSwaps as pending (pending.id)}
													{#if pending.side === 'BUY' && 'strategy' in pending}
														...
														>
															...
															<span
																class="inline-flex items-center gap-1 text-[13px] font-bold text-blu-light"
															>
																{#if pending.amount.type === 'USD'}{formatPriceText(
																		pending.amount.value
																	)}{:else}{formatNumber(String(pending.amount.value))}
																	<ChainIcon chain={trade.chain} class="h-3 w-3" />{/if}
															</span>
															...
															<button
																onclick={(e) => {
																	e.stopPropagation();
																	cancelPendingSwap(trade.id, pending.id);
																}}
									...
									<div class="mt-1 rounded-lg border border-bd/40 bg-s2 p-2 space-y-1 text-[10px]">
										...
										{#if completedTrade.avgClosingPrice?.usd}
											<div class="flex items-center justify-between">
												...
												<span class="font-semibold text-tx"
													>{#if closeMc}{formatMarketCap(closeMc)}{/if}
													<span class="text-g4"
														>(<CurrencyValue
															usd={String(completedTrade.avgClosingPrice.usd)}
															native={String(completedTrade.avgClosingPrice.native)}
															chain={trade.chain}
															mode="price"
															iconClass="h-3 w-3"
														/>)</span
											...
											{#each metTP as met, idx}
												<div class="flex items-center gap-1.5 rounded-lg bg-org/10 px-2.5 py-1.5">
													...
													<span
														...
														>{targetTriggerLabel(met)}</span
													>
													<span class="text-[11px] text-org/50"
														>sold {Math.round(met.sellPct)}%</span
													>
												</div>
											{/each}
											{#each metSL as met, idx}
												<div class="flex items-center gap-1.5 rounded-lg bg-red/10 px-2.5 py-1.5">
													...
													<span
														...
														>{targetTriggerLabel(met)}</span
													>
													<span class="text-[11px] text-red/50"
														>sold {Math.round(met.sellPct)}%</span
													>
												</div>
											{/each}
								...
								>
									{#if activeTab === 'active'}
										<button
											...
											class="btn-secondary px-3 py-1.5 text-xs">Details</button
										>
										<button
											onclick={() => openEditModal(trade as ActiveTrade)}
											class="btn-secondary px-3 py-1.5 text-xs">Edit</button
										>
										<button
											onclick={() => (dismissConfirmTrade = trade as ActiveTrade)}
											class="btn-danger-outline px-3 py-1.5 text-xs">Dismiss</button
										>
										<button
											onclick={() => openSellModal(trade as ActiveTrade)}
											class="btn-danger-outline px-3 py-1.5 text-xs">Sell</button
										>
									{:else if activeTab === 'pending'}
										<button
											onclick={() => (selectedTrade = trade)}
	...
	{#if selectedTrade}
		...
		{@const detailHasTargets =
			detailLiveTakeProfitTargets.length > 0 || detailLiveStopLossTargets.length > 0}
		<div
			use:portal
			class="fixed inset-0 z-[300] flex items-center justify-center overflow-y-auto bg-s0/60 backdrop-blur-[2px] p-3 md:p-6"
			onclick={(e) => {
				if (e.target === e.currentTarget) selectedTrade = null;
			}}
			onkeydown={(e) => {
				if (e.key === 'Escape') selectedTrade = null;
			}}
			role="dialog"
			tabindex="-1"
		>
			<div
				class="w-full max-w-lg rounded-2xl border border-bd bg-s5 shadow-2xl backdrop-blur-xl overflow-x-hidden"
			>
				<div class="flex items-center justify-between border-b border-bd px-4 md:px-5 py-3 md:py-4">
					<div class="flex items-center gap-3">
						<img
							src={tokenImage(trade.chain, trade.tokenAddress)}
							alt=""
							class="h-9 w-9 rounded-xl ring-1 ring-bd"
							onerror={(e) => {
								(e.currentTarget as HTMLImageElement).style.display = 'none';
							}}
						/>
				...
				<div
					class="grid grid-cols-2 {'currentValue' in trade && trade.currentValue
						? 'md:grid-cols-4'
						: 'md:grid-cols-3'} gap-3 border-b border-bd px-4 md:px-5 py-3"
				>
					<div>
						...
						<CurrencyValue
							...
							iconClass="h-3.5 w-3.5"
						/>
						<div class="text-xs text-g4">
							{formatNumber(String(trade.totalBought.tokens))}
							{trade.tokenSymbol}
						</div>
					</div>
					<div>
						...
						/>
						<div class="text-xs text-g4">
							{formatNumber(String(trade.totalSold.tokens))}
							{trade.tokenSymbol}
						</div>
					</div>
					{#if 'currentValue' in trade && trade.currentValue}
						<div>
							...
							<CurrencyValue
								...
								iconClass="h-3.5 w-3.5"
							/>
							{#if trade.pnl.multiplier}<div class="text-xs text-yel">
									{netPnlMultiplier(trade).toFixed(2)}x
								</div>{/if}
						</div>
					{/if}
					<div>
						<div class="text-[10px] font-medium uppercase tracking-wider text-g5">
							PnL <span class="text-g6 normal-case">(net of fees)</span>
						</div>
						<div class="mt-0.5 text-sm font-bold {pnlColor(trade)}">
							<CurrencyValue
								...
								iconClass="h-3.5 w-3.5"
							/>
							{#if trade.pnl.multiplier && !('currentValue' in trade && trade.currentValue)}<span
									class="ml-1 text-xs text-yel">{netPnlMultiplier(trade).toFixed(2)}x</span
								>{/if}
						</div>
								...
								<div class="rounded-xl border border-grn/20 bg-s2 px-3.5 py-2.5">
									<div class="flex items-center justify-between gap-3">
										<div class="flex min-w-0 items-center gap-2">
											<span
												class="rounded-md bg-grn/10 px-1.5 py-0.5 text-[11px] font-bold text-grn"
												>TP{idx + 1}</span
											>
										...
										<span
											class="rounded-md bg-grn/10 px-1.5 py-0.5 text-[10px] font-bold uppercase text-grn"
											>live</span
										>
									</div>
								</div>
							...
							{#each detailLiveStopLossTargets as target, idx}
								<div class="rounded-xl border border-red/20 bg-s2 px-3.5 py-2.5">
									<div class="flex items-center justify-between gap-3">
										<div class="flex min-w-0 items-center gap-2">
											<span
												class="rounded-md bg-red/10 px-1.5 py-0.5 text-[11px] font-bold text-red"
												>SL{idx + 1}</span
											>
											<span class="text-sm font-bold text-red">{targetTriggerLabel(target)}</span>
											{#if target.mode === 'TRAILING'}<span class="text-xs text-red/45">trail</span
												>{/if}
											<span class="text-xs text-red/45">sell {Math.round(target.sellPct)}%</span>
										</div>
										<span
											class="rounded-md bg-red/10 px-1.5 py-0.5 text-[10px] font-bold uppercase text-red"
											>live</span
										>
									</div>
								</div>
				...
				<div class="px-5 py-3">
					<div class="mb-2 text-xs font-semibold uppercase tracking-wider text-g6">
						Swaps ({trade.swaps.length})
					</div>
					{#if trade.swaps.length === 0}
								...
								<div class="rounded-xl border border-bd/40 bg-s2 px-3.5 py-2.5">
									<div class="flex items-center justify-between">
										<div class="flex items-center gap-2">
											<span
												class="rounded-md px-1.5 py-0.5 text-[11px] font-bold {swap.side === 'BUY'
													? 'bg-grn/10 text-grn'
													: 'bg-red/10 text-red'}"
											>
												{swap.side}
											...
											<span
												class="rounded-md px-1.5 py-0.5 text-[10px] {swap.txHash
													? 'bg-grn/10 text-grn'
													: 'bg-yel/10 text-yel'}"
											>
											...
											{#if 'target' in swap && swap.target}
												...
												<span
													class="rounded-md px-1.5 py-0.5 text-[10px] font-bold {st.kind ===
													'TAKE_PROFIT'
														? 'bg-org/10 text-org'
														: 'bg-red/10 text-red'}"
												>
													{st.kind === 'TAKE_PROFIT' ? 'TP' : 'SL'}
													{targetTriggerLabel(st)} hit
												</span>
											{/if}
										</div>
										<span
											class="text-[11px] text-g4 cursor-help"
											title={fullDateTime(swap.timestamp)}>{timeAgo(swap.timestamp, getNow())}</span
										>
									</div>
									<div class="mt-2 grid grid-cols-2 gap-2 text-xs">
										...
										{#if swap.value.amountToken}
											<div>
												...
												<div class="font-medium text-tx">
													{formatNumber(String(swap.value.amountToken))}
													{trade.tokenSymbol}
												</div>
											</div>
										{/if}
									...
									{#if swap.txHash}
										<div class="mt-2 flex items-center justify-between border-t border-bd/40 pt-2">
				...
				<div class="flex items-center justify-between border-t border-bd px-5 py-3">
					<button
						...
						onclick={() => {
							const c = trade.chain;
							const a = trade.tokenAddress;
							selectedTrade = null;
							goto(`/?chain=${c}&token=${a}`);
							onnavigate();
						}}>View Token</button
					>
					<div class="flex items-center gap-2">
						{#if trade.status === 'ACTIVE'}
							<button
								onclick={() => sellAll(trade as ActiveTrade)}
								disabled={sellingTradeId === trade.id}
								class="btn-danger-outline px-4 py-2 text-xs"
								>{sellingTradeId === trade.id ? 'Selling...' : 'Sell All'}</button
							>
						{/if}
						<button
							onclick={() => {
							...
							class="btn-secondary px-3.5 py-2 text-xs">Close</button
						>
					</div>
				</div>
			</div>
	...
	{#if editingTrade}
		...
		<div
			...
			class="fixed inset-0 z-[300] flex items-center justify-center overflow-y-auto bg-s0/60 backdrop-blur-[2px] p-3 md:p-6"
			onclick={(e) => {
				if (e.target === e.currentTarget) editingTrade = null;
			}}
			onkeydown={(e) => {
				if (e.key === 'Escape') editingTrade = null;
			}}
			role="dialog"
			tabindex="-1"
		>
			...
			>
				...
				<div class="max-h-[60vh] overflow-y-auto px-5 py-4">
					<div class="mb-4">
						...
						{#if editTargets.length === 0}
							<div
								class="flex items-center justify-center rounded-lg border border-dashed border-bd/40 py-6 text-xs text-g4"
							>
						...
						{:else}
							<div class="grid max-h-52 grid-cols-2 gap-1.5 overflow-y-auto">
								{#each editTargets as target, i}
									<TargetCard
										target={{
											kind: target.type as SellTargetKind,
											triggerValue: target.trigger,
											sellPercent: target.sellPct,
										...
										onupdate={(t) => {
											editTargets[i] = {
												...editTargets[i],
												type: t.kind,
												trigger: t.triggerValue,
												sellPct: t.sellPercent,
					...
					<div class="mt-4">
						...
						<div class="mt-2 rounded-xl border border-bd bg-s2 p-3">
							<div class="flex items-center gap-2">
								<div class="flex-1">
									<div class="mb-1 text-[10px] font-medium uppercase tracking-wider text-g5">
										Slippage
									</div>
									<div class="relative">
										<input
										...
										/>
										<span
											class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-g4"
											>%</span
										>
									</div>
								</div>
				...
				<div class="flex items-center justify-end gap-2 border-t border-bd px-5 py-3">
					<button onclick={() => (editingTrade = null)} class="btn-secondary px-4 py-2 text-xs"
						>Cancel</button
					>
					<button
	...
	{#if sellModalTrade}
		...
		{@const value = st.currentValue
			? formatUsd(String((st.currentValue.usd * sellPct) / 100))
			: null}
		<div
			use:portal
			class="fixed inset-0 z-[300] flex items-center justify-center bg-s0/60 backdrop-blur-[2px]"
			onclick={(e) => {
				if (e.target === e.currentTarget) sellModalTrade = null;
			}}
			onkeydown={(e) => {
				if (e.key === 'Escape') sellModalTrade = null;
			}}
			role="presentation"
		>
			<div
				class="mx-4 w-full max-w-sm rounded-2xl border border-red/20 bg-s5 p-4 md:p-6 shadow-2xl backdrop-blur-xl"
				onclick={(e) => e.stopPropagation()}
				role="dialog"
				tabindex="-1"
				onkeydown={() => {}}
			>
				<div class="mb-5 flex items-center justify-between">
					<div class="flex items-center gap-3">
						<img
							...
							alt=""
							class="h-9 w-9 rounded-lg ring-1 ring-bd"
							onerror={(e) => {
								(e.currentTarget as HTMLImageElement).style.display = 'none';
							}}
						/>
					...
					<button
						onclick={() => (sellModalTrade = null)}
						class="cursor-pointer rounded-lg p-1.5 text-g5 transition-colors hover:bg-s7 hover:text-tx"
					>
						<X class="h-4 w-4" />
					</button>
				</div>
				...
				<div class="mb-4 flex gap-2">
					{#each [10, 25, 50, 75, 100] as pct}
						<button
							onclick={() => (sellPct = pct)}
							class="flex-1 cursor-pointer rounded-lg border py-2.5 text-sm font-bold transition-all {sellPct ===
							pct
								? 'border-red bg-red/20 text-red'
								: 'border-bd bg-s4 text-g7 hover:text-tx'}"
						>
							{pct}%
						</button>
					{/each}
				...
				<button
					onclick={confirmSell}
					disabled={sellExecuting || sellPct < 1 || sellPct > 100}
					class="btn-danger w-full py-3 text-sm"
					>{sellExecuting ? 'Selling...' : `Sell ${sellPct}%`}</button
				>
			</div>
		</div>
	{/if}
	{#if dismissConfirmTrade}
		<div
			...
			class="fixed inset-0 z-[300] flex items-center justify-center bg-s0/60 backdrop-blur-[2px]"
			onclick={(e) => {
				if (e.target === e.currentTarget) dismissConfirmTrade = null;
			}}
			onkeydown={(e) => {
				if (e.key === 'Escape') dismissConfirmTrade = null;
			}}
			role="presentation"
		>
			<div
				class="w-full max-w-sm mx-3 md:mx-0 rounded-2xl border border-red/20 bg-s5 p-4 md:p-6 shadow-2xl backdrop-blur-xl"
				onclick={(e) => e.stopPropagation()}
				role="dialog"
				tabindex="-1"
				onkeydown={() => {}}
			>
				<div class="mb-4 flex items-center gap-3">
					...
					<div>
						...
						<div class="flex items-center gap-1 text-xs text-g6">
							{dismissConfirmTrade.tokenSymbol} on <ChainIcon
								chain={dismissConfirmTrade.chain}
								class="h-3 w-3 text-g6"
							/>
						</div>
					</div>
				</div>
				<p class="mb-5 text-xs leading-relaxed text-g8">
					This will cancel the position and remove it from the trading engine. Any pending orders
					(buy dips, sell targets, stop losses) will be cancelled. This action cannot be undone.
				</p>
				<div class="flex items-center justify-end gap-2">
					<button
						onclick={() => (dismissConfirmTrade = null)}
						class="btn-secondary px-4 py-2 text-xs">Cancel</button
					>
					<button
						onclick={async () => {
...
{#snippet tradeBadge(trade: ActiveTrade | CompletedTrade)}
	...
	{#if sources.length > 0}
		<div class="mt-1.5 flex flex-wrap items-center gap-1">
			{#each sources as src}
				<span
					class="flex items-center gap-1 rounded bg-grn/10 px-2 py-1 text-[10px] font-medium text-grn"
					title={src.sourceName || 'Bot'}
				>
