...
let activeTradesChain: Chain | undefined;
let completedTradesChain: Chain | undefined;
let activeRequestGeneration = 0;
let completedRequestGeneration = 0;
const activeSeenCursors = new Set<string>();
const completedSeenCursors = new Set<string>();
const completedRestTradeIds = new Set<number>();
const MAX_LIVE_COMPLETED_TRADES = 100;

let managedWallets = $state<Record<string, ManagedWallet>>({});

let sellTargets = $state<SellTargetRow[]>([]);
let stopLoss = $state<StopLossState>({
	enabled: false,
	mode: 'NORMAL',
	kind: 'PERCENTAGE',
	triggerValue: '50',
...
export function subscribeFeeEstimate(chain: Chain) {
	...
	if (feeEstimateWsKey) {
		unsubscribe(feeEstimateWsKey);
		feeEstimateWsKey = null;
	}
...
export function getActiveTradeTab() {
	return activeTradeTab;
}
export function setActiveTradeTab(t: TradeTab) {
	activeTradeTab = t;
}
...
export function setBuyAmount(v: string) {
	buyAmount = v;
}
export function getSellPercent() {
	return sellPercent;
}
export function setSellPercent(v: number) {
	sellPercent = v;
}
export function getBuyGasType() {
	return buyGasType;
}
export function setBuyGasType(v: GasPreset) {
	buyGasType = v;
}
export function getSellGasType() {
	return sellGasType;
}
export function setSellGasType(v: GasPreset) {
	sellGasType = v;
}
export function getSlippageBuy() {
	return slippageBuy;
}
export function setSlippageBuy(v: number | null) {
	slippageBuy = v;
}
export function getSlippageSell() {
	return slippageSell;
}
export function setSlippageSell(v: number | null) {
	slippageSell = v;
}
export function getAntiMev() {
	return antiMev;
}
export function setAntiMev(v: boolean) {
	antiMev = v;
}
export function getDipPercent() {
	return dipPercent;
}
export function setDipPercent(v: string) {
	dipPercent = v;
}
export function getLimitPrice() {
	return limitPrice;
}
export function setLimitPrice(v: string) {
	limitPrice = v;
}
...
export function getCompletedFetched() {
	return completedFetched;
}
export function getActiveLoading() {
	return activeLoading;
}
export function getCompletedLoading() {
	return completedLoading;
}
export function getActiveHasMore() {
	return activeHasMore;
}
export function getCompletedHasMore() {
	return completedHasMore;
}
export function getActiveCursor() {
	return activeCursor;
}
export function getCompletedCursor() {
	return completedCursor;
}
export function getActiveCursorTriplet(): CursorTriplet {
	return { cursor: activeCursor, prevCursor: activePrevCursor, nextCursor: activeNextCursor };
}
export function getCompletedCursorTriplet(): CursorTriplet {
	return {
		cursor: completedCursor,
		prevCursor: completedPrevCursor,
		nextCursor: completedNextCursor
	};
}
export function getActiveTradesChain() {
	return activeTradesChain;
}
export function getCompletedTradesChain() {
	return completedTradesChain;
}
export function getActiveNextCursor() {
	return activeNextCursor;
}
export function getCompletedNextCursor() {
	return completedNextCursor;
}
export function getBuyLoading() {
	return buyLoading;
}
export function getSellLoading() {
	return sellLoading;
}
export function getTradeError() {
	return tradeError;
}
export function clearTradeError() {
	tradeError = null;
}
...
function tradeFailureSideFromPendingSwap(
	tradeIds: number[],
	swapIds: number[]
): TradeFailureSide | undefined {
...
function tradeFailureSide(
	...
	const direct =
		normalizeTradeSide(detail?.side) ??
...
export function getSellTargets() {
	return sellTargets;
}
export function addSellTarget(
	targetKind: TargetKind = 'TAKE_PROFIT',
...
export function removeSellTarget(idx: number) {
	sellTargets = sellTargets.filter((_, i) => i !== idx);
}
export function updateSellTarget(idx: number, target: SellTargetRow) {
	sellTargets = sellTargets.map((t, i) => (i === idx ? target : t));
}
export function getStopLoss() {
	return stopLoss;
}
export function updateStopLossField<K extends keyof StopLossState>(
	key: K,
	value: StopLossState[K]
) {
	stopLoss = { ...stopLoss, [key]: value };
}

export interface TradeConfigEntry {
	id: TradePresetSlot;
	name: string;
	preset: TradePreset;
}
...
export function getTradeConfigsLoading() {
	return tradePresetsLoading;
}
export function getSelectedConfigId() {
	return selectedPresetSlot;
}
export function setSelectedConfigId(id: string | null) {
	selectedPresetSlot = id as TradePresetSlot | null;
}
export function getTradeConfigsLimitRemaining() {
	return 0;
}
...
export function loadConfigIntoForm(entry: TradeConfigEntry) {
	...
	else if (p.strategy.type === 'DIP') {
		orderType = 'DIP';
		dipPercent = String(p.strategy.dipPct);
	} else if (p.strategy.type === 'LIMIT') {
		orderType = 'LIMIT';
		limitPrice = String(p.strategy.priceUsd);
	}
	...
	sellTargets = s.targets.map((t) => {
		...
		let triggerValue = '';
		if (trigger.type === 'MULTIPLIER') {
			kind = 'MULTIPLE';
			triggerValue = String((trigger as any).multiplier);
		} else if (trigger.type === 'PERCENT') {
			kind = 'PERCENTAGE';
			triggerValue = String((trigger as any).changePct);
		} else if (trigger.type === 'PRICE') {
			kind = 'USD';
			triggerValue = String((trigger as any).priceUsd);
		} else if (trigger.type === 'MARKET_CAP_USD') {
			kind = 'MARKETCAP';
			triggerValue = String((trigger as any).marketCapUsd);
		}
		return {
			kind,
			triggerValue,
...
export function getCompletedTotalCount(): number {
	return completedTotalCount != null
		? Math.max(completedTotalCount, completedTrades.length)
...
function isTradeWsTrade(value: unknown): value is TradeWsTrade {
	...
	return (
		...
		(trade.status === 'ACTIVE' || trade.status === 'PENDING' || trade.status === 'COMPLETED')
	);
}
...
function removeTradeIds(ids: unknown, scope?: TradeWsScope) {
	...
	if (!scope || scope === 'COMPLETED')
		completedTrades = completedTrades.filter((t) => !idSet.has(t.id));
}
...
function upsertTrade<T extends ActiveTrade | CompletedTrade>(trades: T[], updated: T): T[] {
	...
	return [...trades.slice(0, idx), { ...trades[idx], ...updated }, ...trades.slice(idx + 1)];
}
...
function assignActiveCursorTriplet(page: CursorTriplet | undefined | null) {
	...
	activeNextCursor = unseenCursor(page?.nextCursor, activeSeenCursors);
	activeHasMore = activeNextCursor !== undefined;
}
...
function assignCompletedCursorTriplet(page: CursorTriplet | undefined | null) {
	...
	completedNextCursor = unseenCursor(page?.nextCursor, completedSeenCursors);
	completedHasMore = completedNextCursor !== undefined;
}

function mergeActiveTrades(current: ActiveTrade[], incoming: ActiveTrade[]): ActiveTrade[] {
	return mergeUniqueById(current, incoming);
}

function reconcileActiveTotals() {
	const loadedActive = activeTrades.filter((trade) => trade.status === 'ACTIVE').length;
	const loadedPending = activeTrades.filter((trade) => trade.status === 'PENDING').length;
	activePositionsTotal = totalAtLeastLoaded(activePositionsTotal, loadedActive);
	pendingTradesTotal = totalAtLeastLoaded(pendingTradesTotal, loadedPending);
}

function reconcileCompletedTotal() {
	completedTotalCount = totalAtLeastLoaded(completedTotalCount, completedTrades.length);
}

function mergeCompletedSnapshot(snapshot: CompletedTrade[]) {
	const snapshotById = new Map(snapshot.map((trade) => [trade.id, trade]));
	const restRows = completedTrades
		.filter((trade) => completedRestTradeIds.has(trade.id))
		.map((trade) => snapshotById.get(trade.id) ?? trade);
	const liveRows = mergeUniqueById(
		completedTrades.filter((trade) => !completedRestTradeIds.has(trade.id)),
		snapshot.filter((trade) => !completedRestTradeIds.has(trade.id))
	).slice(0, MAX_LIVE_COMPLETED_TRADES);
	completedTrades = sortCompletedTrades([...liveRows, ...restRows]);
}

export async function fetchActiveTrades(tokenAddress?: string, chain?: Chain) {
	if (!getIsLoggedIn()) return;
	const generation = ++activeRequestGeneration;
	activeTradesChain = chain;
	activeSeenCursors.clear();
	activeLoading = true;
	try {
		const { data } = await api.GET('/v2/trade/active', { params: { query: { chain } } });
		if (generation !== activeRequestGeneration || chain !== activeTradesChain) return;
		activeTrades = mergeActiveTrades([], data?.trades ?? []);
		activePositionsTotal = data?.totalActiveCount ?? null;
		pendingTradesTotal = data?.totalPendingCount ?? null;
		reconcileActiveTotals();
		...
		if (generation === activeRequestGeneration) activeLoading = false;
	}
}

export async function fetchCompletedTrades(tokenAddress?: string, chain?: Chain) {
	if (!getIsLoggedIn()) return;
	const generation = ++completedRequestGeneration;
	completedTradesChain = chain;
	completedSeenCursors.clear();
	completedRestTradeIds.clear();
	completedLoading = true;
	try {
		const { data } = await api.GET('/v2/trade/completed', { params: { query: { chain } } });
		if (generation !== completedRequestGeneration || chain !== completedTradesChain) return;
		completedTrades = sortCompletedTrades(mergeCompletedTrades([], data?.trades ?? []));
		for (const trade of data?.trades ?? []) completedRestTradeIds.add(trade.id);
		completedTotalCount = data?.totalCount ?? null;
		reconcileCompletedTotal();
		...
		if (generation === completedRequestGeneration) completedLoading = false;
	}
}

export async function fetchMoreActive() {
	if (!activeHasMore || activeLoading || !activeNextCursor) return;
	const requestedCursor = activeNextCursor;
	if (activeSeenCursors.has(requestedCursor)) return;
	activeSeenCursors.add(requestedCursor);
	const generation = activeRequestGeneration;
	activeLoading = true;
	try {
		const { data } = await api.GET('/v2/trade/active', {
			params: { query: { cursor: requestedCursor, chain: activeTradesChain } }
		});
		if (generation !== activeRequestGeneration) return;
		activeTrades = mergeActiveTrades(activeTrades, data?.trades ?? []);
		activePositionsTotal = data?.totalActiveCount ?? activePositionsTotal;
		pendingTradesTotal = data?.totalPendingCount ?? pendingTradesTotal;
		reconcileActiveTotals();
		assignActiveCursorTriplet(data);
	} catch {
		activeSeenCursors.delete(requestedCursor);
	} finally {
		if (generation === activeRequestGeneration) activeLoading = false;
	}
}
...
export async function fetchMoreCompleted() {
	...
	const requestedCursor = completedNextCursor;
	if (completedSeenCursors.has(requestedCursor)) return;
	completedSeenCursors.add(requestedCursor);
	const generation = completedRequestGeneration;
	completedLoading = true;
	try {
		const { data } = await api.GET('/v2/trade/completed', {
			params: { query: { cursor: requestedCursor, chain: completedTradesChain } }
		});
		if (generation !== completedRequestGeneration) return;
		completedTrades = mergeCompletedTrades(completedTrades, data?.trades ?? []);
		for (const trade of data?.trades ?? []) completedRestTradeIds.add(trade.id);
		completedTotalCount = data?.totalCount ?? completedTotalCount;
		reconcileCompletedTotal();
		for (const t of data?.trades ?? []) markCompleted(t.id);
		assignCompletedCursorTriplet(data);
	} catch {
		completedSeenCursors.delete(requestedCursor);
	} finally {
		if (generation === completedRequestGeneration) completedLoading = false;
	}
}
...
function rowToTrigger(row: SellTargetRow): TradeTargetTrigger {
	...
	switch (row.kind) {
		case 'MULTIPLE':
			return { type: 'MULTIPLIER', multiplier: val } as TradeTargetTrigger;
		case 'PERCENTAGE':
			return positivePercentTargetTrigger(val);
		case 'MARKETCAP':
			return { type: 'MARKET_CAP_USD', marketCapUsd: val } as TradeTargetTrigger;
		case 'USD':
			return { type: 'PRICE', priceUsd: val } as TradeTargetTrigger;
		default:
			return { type: 'MULTIPLIER', multiplier: val } as TradeTargetTrigger;
	}
}
	...
	if (stopLoss.enabled) {
		...
		if (!isNaN(val) && !isNaN(sellPct) && sellPct > 0) {
			...
			switch (stopLoss.kind) {
				case 'PERCENTAGE':
					trigger = positivePercentTargetTrigger(val);
					break;
				case 'PRICE':
					trigger = { type: 'PRICE', priceUsd: val } as TradeTargetTrigger;
					break;
				case 'MARKETCAP':
					trigger = { type: 'MARKET_CAP_USD', marketCapUsd: val } as TradeTargetTrigger;
					break;
				default:
					trigger = positivePercentTargetTrigger(val);
			}
...
export function handleTradesSnapshot(
	topic: string,
	data: ActiveTradesResponse | CompletedTradesResponse | unknown,
	...
	activeTrades = mergeActiveTrades(
		activeTrades,
		trades.filter(
			(trade): trade is ActiveTrade => trade.status === 'ACTIVE' || trade.status === 'PENDING'
		)
	);
...
export function handleBalanceUpdate(data: { balanceUpdate?: BalanceUpdatePayload } | unknown) {
	...
	for (const a of update.assets) {
		const idx = updatedAssets.findIndex((wa) => wa.token.address === a.token.address);
		if (idx >= 0) {
			updatedAssets[idx] = a;
		} else {
	...
	const base: ManagedWallet = existing ?? {
		address: update.walletAddress,
		assets: [],
		totalValueUsd: 0
	};
	managedWallets = {
...
export function handleTradeUpdate(
	event: string,
	...
	function pendingSummary(): string | undefined {
		...
		if (p?.side === 'BUY' && p?.amount) {
			const amt =
				p.amount.type === 'USD'
					? formatUsd(p.amount.value)
					: `${fmtNative(p.amount.value)} ${nativeSym}`;
	...
	switch (type) {
		case 'SWAP_SUBMITTED':
			addToast(
				'info',
				`Swap Submitted${tokenLabel}`,
				pendingSummary() ?? 'Tx sent to network',
				4000,
				tradeDetails()
			);
			break;
		...
		case 'SWAP_FINALIZED':
			...
			// is gone (dismissed / this client subscribed mid-flight).
			{
				...
				if (!summary) break;
				const existingId =
					detail?.swapId != null ? confirmedSwapToasts.get(detail.swapId) : undefined;
		...
		case 'SWAP_FAILED':
			{
				...
				addToast(
					'error',
					`${tradeFailureTitle(side)}${tokenLabel}`,
					msg,
					8000,
					tradeErrorDetails()
				);
